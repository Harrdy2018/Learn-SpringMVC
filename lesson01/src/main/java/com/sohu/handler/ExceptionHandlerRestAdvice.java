...
/**
 * ```@ControllerAdvice注解里面包含@Component注解```
 * ```@RestControllerAdvice是@ControllerAdvice和@ResponseBody结合体```
 */
@RestControllerAdvice(annotations = {CustomizedHandleException.class})
public class ExceptionHandlerRestAdvice {
    private static final Logger LOGGER = LoggerFactory.getLogger(ExceptionHandlerRestAdvice.class);
